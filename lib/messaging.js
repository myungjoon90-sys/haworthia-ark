...
// 관리자(명준님) 본인에게 알림 문자 발송 — 새 제출/가입 등
//  받는 번호: ADMIN_PHONE 환경변수 (없으면 SENDER_PHONE 으로 발송)
async function sendAdminAlert(text) {
  try {
    requiredEnv();
    const to = digits(process.env.ADMIN_PHONE || process.env.SENDER_PHONE);
    await sendViaSolapi({
      to, from: digits(process.env.SENDER_PHONE),
      type: 'LMS', subject: '[방주] 알림', text,
    });
    return true;
  } catch (e) {
    console.warn('관리자 알림 발송 실패(무시):', e.message);
    return false;
  }
}
